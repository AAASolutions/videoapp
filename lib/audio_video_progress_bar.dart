...
class ProgressBar extends LeafRenderObjectWidget {
  ...
  const ProgressBar({
    ...
    required this.mark_start,
    required this.mark_end,
    required this.total,
    this.buffered,
    this.onSeek,
    this.onMarkStartDragDrop,
    this.onMarkStartDragStart,
    this.onMarkStartDragUpdate,
    this.onMarkStartDragEnd,
    this.onMarkEndDragDrop,
    this.onMarkEndDragStart,
    this.onMarkEndDragUpdate,
    this.onMarkEndDragEnd,
    this.onDragStart,
    this.onDragUpdate,
    this.onDragEnd,
    this.barHeight = 5.0,
  ...
  final Duration mark_start;
  final Duration mark_end;
  ...
  final ValueChanged<Duration>? onMarkStartDragDrop;
  final ValueChanged<Duration>? onMarkEndDragDrop;
  ...
  final ThumbDragStartCallback? onMarkStartDragStart;
  final ThumbDragStartCallback? onMarkEndDragStart;
  ...
  final ThumbDragUpdateCallback? onMarkStartDragUpdate;
  final ThumbDragUpdateCallback? onMarkEndDragUpdate;
  ...
  final VoidCallback? onMarkStartDragEnd;
  final VoidCallback? onMarkEndDragEnd;
  ...
  RenderObject createRenderObject(BuildContext context) {
    ...
    return _RenderProgressBar(
      ...
      onDragEnd: onDragEnd,
      onMarkStartDragDrop: onMarkStartDragDrop,
      onMarkStartDragStart: onMarkStartDragStart,
      onMarkStartDragUpdate: onMarkStartDragUpdate,
      onMarkStartDragEnd: onMarkStartDragEnd,
      onMarkEndDragDrop: onMarkEndDragDrop,
      onMarkEndDragStart: onMarkEndDragStart,
      onMarkEndDragUpdate: onMarkEndDragUpdate,
      onMarkEndDragEnd: onMarkEndDragEnd,
      barHeight: barHeight,
  ...
  void updateRenderObject(BuildContext context, RenderObject renderObject) {
    ...
    (renderObject as _RenderProgressBar)
      ..progress = progress
      ..total = total
      .._mark_start = mark_start
      .._mark_end = mark_end
      .._markStartValue = renderObject._proportionOfTotal(mark_start)
      .._markEndValue = renderObject._proportionOfTotal(mark_end)
      ..buffered = buffered ?? Duration.zero
      ..onSeek = onSeek
      ..onDragStart = onDragStart
      ..onDragUpdate = onDragUpdate
...
class _RenderProgressBar extends RenderBox {
  _RenderProgressBar({
    ...
    required Duration mark_start,
    required Duration mark_end,
    ...
    ValueChanged<Duration>? onMarkStartDragDrop,
    ThumbDragStartCallback? onMarkStartDragStart,
    ThumbDragUpdateCallback? onMarkStartDragUpdate,
    VoidCallback? onMarkStartDragEnd,
    ValueChanged<Duration>? onMarkEndDragDrop,
    ThumbDragStartCallback? onMarkEndDragStart,
    ThumbDragUpdateCallback? onMarkEndDragUpdate,
    VoidCallback? onMarkEndDragEnd,
    ...
    double textScaleFactor = 1.0,
        ...
        _onMarkStartDragDrop = onMarkStartDragDrop,
        _onMarkStartDragStartUserCallback = onMarkStartDragStart,
        _onMarkStartDragUpdateUserCallback = onMarkStartDragUpdate,
        _onMarkStartDragEndUserCallback = onMarkStartDragEnd,
    ...
    _markStartValue = _proportionOfTotal(mark_start);
    _markEndValue = _proportionOfTotal(mark_end);
  ...
  late double _markStartValue;
  late double _markEndValue;

  Duration _mark_start;
  Duration _mark_end;
  ...
  bool _userIsDraggingMarkStart =  false;
  bool _userIsDraggingMarkEnd =  false;
  ...
  void _onDragStart(DragStartDetails details) {
    if( _userIsDraggingThumb == false &&
        _userIsDraggingMarkStart == false &&
        _userIsDraggingMarkEnd == false)
    {
        if(((details.localPosition.dy / size.height) < 0.21) && 
        (((details.localPosition.dx / size.width) - _markEndValue).abs() < 0.01))
        {
        _userIsDraggingMarkEnd = true;
        _updateMarkEndPosition(details.localPosition);
        onMarkEndDragStart?.call(ThumbDragDetails(
            timeStamp: _currentMarkEndDuration(),
            globalPosition: details.globalPosition,
            localPosition: details.localPosition,
        ));
        }
        else if(((details.localPosition.dy / size.height) < 0.21) && 
        (((details.localPosition.dx / size.width) - _markStartValue).abs() < 0.01))
        {
        _userIsDraggingMarkStart = true;
        _updateMarkStartPosition(details.localPosition);
        onMarkStartDragStart?.call(ThumbDragDetails(
            timeStamp: _currentMarkStartDuration(),
            globalPosition: details.globalPosition,
            localPosition: details.localPosition,
        ));
        }
        else if(((details.localPosition.dx / size.width) - _proportionOfTotal(progress)).abs() < 0.10)
        {
  ...
  void _onDragUpdate(DragUpdateDetails details) {
    ...
    else if(_userIsDraggingMarkStart)
    {
      if((details.localPosition.dx / size.width) < (_markEndValue - 0.01))
      {
        _updateMarkStartPosition(details.localPosition);
        onMarkStartDragUpdate?.call(ThumbDragDetails(
          timeStamp: _currentMarkStartDuration(),
          globalPosition: details.globalPosition,
          localPosition: details.localPosition,
        ));
        onMarkStartDragDrop?.call(_currentMarkStartDuration());
      }     
    }
    else if(_userIsDraggingMarkEnd)
    {
      if((details.localPosition.dx / size.width) > (_markStartValue + 0.01))
      {
        _updateMarkEndPosition(details.localPosition);
        onMarkEndDragUpdate?.call(ThumbDragDetails(
            timeStamp: _currentMarkEndDuration(),
            globalPosition: details.globalPosition,
            localPosition: details.localPosition,
        ));
        onMarkEndDragDrop?.call(_currentMarkEndDuration());
      }
    }
  }
  ...
  void _onDragEnd(DragEndDetails details) {
    ...
    {
        onMarkStartDragEnd?.call();
        onMarkStartDragDrop?.call(_currentMarkStartDuration());
    }
    else if(_userIsDraggingMarkEnd)
    {
        onMarkEndDragEnd?.call();
        onMarkEndDragDrop?.call(_currentMarkEndDuration());
    }
  ...
  void _finishDrag() {
    ...
    if(_userIsDraggingMarkStart)
    {
      _userIsDraggingMarkStart = false;
    }


    if(_userIsDraggingMarkEnd)
    {
      _userIsDraggingMarkEnd = false;
    }
  ...
  void _onMarkStartDragStart(DragStartDetails details) {
    _updateMarkStartPosition(details.localPosition);
    onMarkStartDragStart?.call(ThumbDragDetails(
      timeStamp: _currentMarkStartDuration(),
      globalPosition: details.globalPosition,
  ...
  void _onMarkStartDragUpdate(DragUpdateDetails details) {
    _updateMarkStartPosition(details.localPosition);
    onMarkStartDragUpdate?.call(ThumbDragDetails(
      timeStamp: _currentMarkStartDuration(),
      globalPosition: details.globalPosition,
  ...
  void _onMarkStartDragEnd(DragEndDetails details) {
    onMarkStartDragEnd?.call();
    onMarkStartDragDrop?.call(_currentMarkStartDuration());
    markNeedsPaint();
  }
  
  void _onMarkEndDragStart(DragStartDetails details) {
    _updateMarkEndPosition(details.localPosition);
    onMarkEndDragStart?.call(ThumbDragDetails(
      timeStamp: _currentMarkEndDuration(),
      globalPosition: details.globalPosition,
  ...
  void _onMarkEndDragUpdate(DragUpdateDetails details) {
    _updateMarkEndPosition(details.localPosition);
    onMarkEndDragUpdate?.call(ThumbDragDetails(
      timeStamp: _currentMarkEndDuration(),
      globalPosition: details.globalPosition,
  ...
  void _onMarkEndDragEnd(DragEndDetails details) {
    onMarkEndDragEnd?.call();
    onMarkEndDragDrop?.call(_currentMarkEndDuration());
    markNeedsPaint();
  }
  ...
  Duration _currentMarkStartDuration() {
    final thumbMilliseconds = _markStartValue * total.inMilliseconds;
    return Duration(milliseconds: thumbMilliseconds.round());
  }
  Duration _currentMarkEndDuration() {
    final thumbMilliseconds = _markEndValue * total.inMilliseconds;
    return Duration(milliseconds: thumbMilliseconds.round());
  }
  ...
  void _updateMarkStartPosition(Offset localPosition) {
    final dx = localPosition.dx;
    double lengthBefore = 0.0;
    double lengthAfter = 0.0;
    if (_timeLabelLocation == TimeLabelLocation.sides) {
    ...
    final barWidth = barEnd - barStart;
    final position = (dx - barStart).clamp(0.0, barWidth);
    _markStartValue = (position / barWidth);
    _mark_start = _currentMarkStartDuration();
    markNeedsPaint();
  }

  void _updateMarkEndPosition(Offset localPosition) {
    final dx = localPosition.dx;
    double lengthBefore = 0.0;
    double lengthAfter = 0.0;
    if (_timeLabelLocation == TimeLabelLocation.sides) {
    ...
    final barWidth = barEnd - barStart;
    final position = (dx - barStart).clamp(0.0, barWidth);
    _markEndValue = (position / barWidth);
     _mark_end = _currentMarkEndDuration();
    markNeedsPaint();
  }
  ...
  TextPainter _totalTimeLabel() {
    final text = _getTimeString(total);
    return _layoutText(text);
  }

  TextPainter _markTimeLabel() {
    final text = _getTimeString(_mark_end - _mark_start);
    return _layoutText(text);
  }
  ...
  TextPainter _markStartTimeLabel() {
    final text = _getTimeString(_mark_start);
    return _layoutText(text);
  }

  TextPainter _markEndTimeLabel() {
    final text = _getTimeString(_mark_end);
    return _layoutText(text);
  }
  ...
  ValueChanged<Duration>? get onMarkStartDragDrop => _onMarkStartDragDrop;
  ValueChanged<Duration>? _onMarkStartDragDrop;
  set onMarkStartDragDrop(ValueChanged<Duration>? value) {
    if (value == _onMarkStartDragDrop) {
      return;
    }
    _onMarkStartDragDrop = value;
  }
  ValueChanged<Duration>? get onMarkEndDragDrop => _onMarkEndDragDrop;
  ValueChanged<Duration>? _onMarkEndDragDrop;
  set onMarkEndDragDrop(ValueChanged<Duration>? value) {
    if (value == _onMarkEndDragDrop) {
      return;
    }
    _onMarkEndDragDrop = value;
  }
  ...
  ThumbDragStartCallback? get onMarkStartDragStart => _onMarkStartDragStartUserCallback;
  ThumbDragStartCallback? _onMarkStartDragStartUserCallback;
  set onMarkStartDragStart(ThumbDragStartCallback? value) {
    if (value == _onMarkStartDragStartUserCallback) {
      return;
    }
    _onMarkStartDragStartUserCallback = value;
  }
  ThumbDragStartCallback? get onMarkEndDragStart => _onMarkEndDragStartUserCallback;
  ThumbDragStartCallback? _onMarkEndDragStartUserCallback;
  set onMarkEndDragStart(ThumbDragStartCallback? value) {
    if (value == _onMarkEndDragStartUserCallback) {
      return;
    }
    _onMarkEndDragStartUserCallback = value;
  }
  ...
  ThumbDragUpdateCallback? get onMarkStartDragUpdate => _onMarkStartDragUpdateUserCallback;
  ThumbDragUpdateCallback? _onMarkStartDragUpdateUserCallback;
  set onMarkStartDragUpdate(ThumbDragUpdateCallback? value) {
    if (value == _onMarkStartDragUpdateUserCallback) {
      return;
    }
    _onDragUpdateUserCallback = value;
  }
  ThumbDragUpdateCallback? get onMarkEndDragUpdate => _onMarkEndDragUpdateUserCallback;
  ThumbDragUpdateCallback? _onMarkEndDragUpdateUserCallback;
  set onMarkEndDragUpdate(ThumbDragUpdateCallback? value) {
    if (value == _onMarkEndDragUpdateUserCallback) {
      return;
    }
    _onDragUpdateUserCallback = value;
  }
  ...
  VoidCallback? get onMarkStartDragEnd => _onMarkStartDragEndUserCallback;
  VoidCallback? _onMarkStartDragEndUserCallback;
  set onMarkStartDragEnd(VoidCallback? value) {
    if (value == _onMarkStartDragEndUserCallback) {
      return;
    }
    _onMarkStartDragEndUserCallback = value;
  }
  VoidCallback? get onMarkEndDragEnd => _onMarkEndDragEndUserCallback;
  VoidCallback? _onMarkEndDragEndUserCallback;
  set onMarkEndDragEnd(VoidCallback? value) {
    if (value == _onMarkEndDragEndUserCallback) {
      return;
    }
    _onMarkEndDragEndUserCallback = value;
  }
  ...
  double _heightWhenLabelsAboveOrBelow() {
    return _heightWhenNoLabels() + _textHeight() + _timeLabelPadding + 25;
  }
  ...
  void _drawProgressBarWithLabelsAboveOrBelow(Canvas canvas) {
    ...
    final leftLabelOffset = Offset(0, labelDy + 25);
    ...
    final rightLabelOffset = Offset(rightLabelDx, labelDy + 25);
  ...
  void _drawProgressBar(Canvas canvas, Offset offset, Size localSize) {
    canvas.save();
    canvas.translate(offset.dx, offset.dy + 25);
    _drawBaseBar(canvas, localSize);
    _drawBufferedBar(canvas, localSize);
    _drawCurrentProgressBar(canvas, localSize);
    _drawMarkStart(canvas, localSize);
    _drawMarkEnd(canvas, localSize);
    _drawThumb(canvas, localSize);

    _markTimeLabel().paint(canvas, Offset(size.width + 20, -5));
    _totalTimeLabel().paint(canvas, Offset(-60, -5));
  ...
  void _drawMarkStart(Canvas canvas, Size localSize) {
    final thumbPaint = Paint()..color = Color.fromARGB(255, 100, 81, 19);
    final barCapRadius = _barHeight / 2;
    final availableWidth = localSize.width - _barHeight;
    var thumbDx = _markStartValue * availableWidth + barCapRadius;
    if (!_thumbCanPaintOutsideBar) {
      thumbDx = thumbDx.clamp(_thumbRadius, localSize.width - _thumbRadius);
    }
    final center = Offset(thumbDx, localSize.height / 2);
    
    canvas.drawLine(center + Offset(0, -25), center, thumbPaint..strokeWidth = 5);
    canvas.drawCircle(center + Offset(0, -25), thumbRadius - 2, thumbPaint);

    if(_userIsDraggingMarkStart)
    {
      _markStartTimeLabel().paint(canvas, center + Offset(-12, -56));
    }
  }
  
  void _drawMarkEnd(Canvas canvas, Size localSize) {
    final thumbPaint = Paint()..color = Color.fromARGB(255, 100, 81, 19);
    final barCapRadius = _barHeight / 2;
    final availableWidth = localSize.width - _barHeight;
    var thumbDx = _markEndValue * availableWidth + barCapRadius;
    if (!_thumbCanPaintOutsideBar) {
      thumbDx = thumbDx.clamp(_thumbRadius, localSize.width - _thumbRadius);
    }
    final center = Offset(thumbDx, localSize.height / 2);
    ...
    if(_userIsDraggingMarkEnd)
    {
      _markEndTimeLabel().paint(canvas, center + Offset(-12, -56));
    }
  }
